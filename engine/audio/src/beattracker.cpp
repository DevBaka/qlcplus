/*
  ...
  Beat tracker by varghele. See beattracker.h for the class
  overview and beattracker.md for the full algorithm
...
BeatTracker::BeatTracker(int sampleRate, int channels)
    : m_sampleRate(sampleRate > 0 ? sampleRate : 44100)
...
void BeatTracker::setFormat(int sampleRate, int channels)
{
    if (sampleRate <= 0)
        return;
